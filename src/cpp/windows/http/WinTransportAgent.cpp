...
char* WinTransportAgent::sendMessage(const char* msg) {
	...
	if (auth) {
		addAuthenticationHeaders(request, headers);
	}
 ...
 */
void WinTransportAgent::addAuthenticationHeaders(HINTERNET hRequest, WCHAR* headers) {
	DWORD dwStatus;
	...
	if (fRet) {
		...
		wcscat(headers, L"\r\nAuthorization: ");
		wcscat(headers, toWideChar(authresponse.c_str()));
	}
}
